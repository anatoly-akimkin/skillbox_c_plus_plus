...
// Что оценивается. В решении не используются условные конструкции.

#include <iostream>

void initialize_matrix(int matrix[][5])
{
    int val = 0;
    int multiplier = -1;
    for (int i = 0; i < 5; i++)
    {
        ...
        // First option
        // for (int j = (-2)*(-1 + multiplier); (j)*multiplier <= 2*(1 + multiplier); j += multiplier)
        // {
        //     matrix[i][j] = val;
        //     val++;
        // }
        ...
        for (int j = 0; j < 5; j++)
        {
            matrix[i][j*multiplier + (-2)*(-1 + multiplier)] = val;
            val++;
        }
    }
}
